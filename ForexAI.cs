  ...
  public class ForexAI : MqlApi
  {
    ...
    public override int start()
    {
      ...
      if(!IsOptimization())
      {
        ...
        if(runWatch.ElapsedMilliseconds - lastMemoryStatsDumpTimesamp >= 9380000)
        {
    ...
    public void InitNetwork()
    {
      ...
      if(Data.networksDirectories.Length > 0)
      {
        ...
        consolelog($"Init network: selected {network} from {Configuration.rootDirectory} (total {Data.networksDirectories.Length} networks)", "debug");
            ...
            if(newStopLoss > OrderOpenPrice() || order.stopLoss == 0)
            {
              ...
              if(newStopLoss != order.stopLoss)
              {
                   ...
                   order.expiration, Color.BlueViolet))
                {
                  ...
                  AddLabel("x", Color.Blue, newStopLoss);
                }
                else
                {
                  currentDayOperationsCount++;
            ...
            if(newStopLoss < OrderOpenPrice() || order.stopLoss == 0)
            {
              ...
              if(newStopLoss != order.stopLoss)
              {
                   ...
                   order.expiration, Color.MediumVioletRed))
                {
                  int error = GetLastError();
                  result = $"FAIL: {error}: {ErrorDescription(error)}";
                  AddLabel("x", Color.Red, newStopLoss);
                }
                else
                {
                  currentDayOperationsCount++;
    ...
    void AddLabel(string text, Color clr, double inPos = 0.0)
    {
      string on;
      double pos = inPos > 0.0 ? inPos : (Bid + (Bid - Ask) / 2);
    ...
    void DrawStats()
    {
      ...
      for(i = 0; i < 10; i++)
      {
        ...
        if(ObjectFind(labelID) == -1)
        {
          ...
          ObjectSet(labelID, OBJPROP_XDISTANCE, 1442);
